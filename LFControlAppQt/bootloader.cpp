...
Bootloader::Bootloader(BluetoothHandler *bluetoothHandler, QObject *parent)
    : QObject(parent), bluetoothHandler(bluetoothHandler), baseAddress(0)
{
...
bool Bootloader::parseHexLine(const QString &line)
{
    ...
    case iHexRecordType::Data:
        for (auto i = 0; i < dataBytes.size(); ++i)
        {
            quint32 addr = baseAddress + address + i;
            if (addr >= APP_START_ADDRESS && addr < APP_END_ADDRESS - CRC_SIZE)
            {
...
void Bootloader::prepareFirmwareData()
{
    totalBytesToFlash = firmwareMap.size();
    crcValue = calculateCRC32();
}

quint32 Bootloader::calculateCRC32()
{
    ...
    for (quint32 addr = APP_START_ADDRESS; addr < APP_END_ADDRESS - CRC_SIZE + 1; ++addr)
    {
        quint8 byte = firmwareMap.value(addr, 0xFF);
        crc = (crc << 8) ^ crc32Table[((crc >> 24) ^ byte) & 0xFF];
    }
...
void Bootloader::sendNextFirmwareChunk()
{
    ...
    {
        QByteArray crcData;
        QDataStream out(&crcData, QIODevice::WriteOnly);
        out.setByteOrder(QDataStream::LittleEndian);
        out << crcValue;

        bluetoothHandler->sendCommand(Command::BootFlashCrc, crcData);
        emit bootloaderMessage("Firmware flashing completed. CRC sent.");
...
void Bootloader::handleDataReceived(Command command, const QByteArray &data)
{
    ...
    case Command::BootFlashData:
        sendNextFirmwareChunk();
        break;
    case Command::BootFlashCrc:
        validateApplication();
        break;
    case Command::BootValidateApp:
        emit bootloaderMessage("Application validated successfully.");
        break;
    default:
        break;
    }
